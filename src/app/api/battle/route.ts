...
// ---------------------------------------------------------------------------
async function getRandomPair(excludePostIds: string[], userId?: string) {
  // Build where clause
  const where: any = {
    status: "PUBLISHED",
    visibility: "PUBLIC",
...
export async function GET(request: NextRequest) {
  try {
    ...
    const pair = await getRandomPair(recentPostIds, user?.id);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const nextBattle = await getRandomPair(recentPostIds, user.id);

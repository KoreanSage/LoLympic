import type { TranslationKeys } from "./en";

const ja: Record<TranslationKeys, string> = {
  // ── Navigation ─────────────────────────────────────────────────────────────
  "nav.explore": "探索",
  "nav.leaderboard": "ランキング",
  "nav.season": "シーズン",
  "nav.upload": "投稿",
  "nav.admin": "管理",
  "nav.search": "検索",
  "nav.searchMemes": "ミームを検索...",
  "nav.login": "ログイン",
  "nav.logout": "ログアウト",
  "nav.settings": "設定",
  "nav.messages": "メッセージ",
  "nav.bookmarks": "ブックマーク",
  "nav.menu": "メニュー",
  "nav.dashboard": "ダッシュボード",

  // ── Notifications ──────────────────────────────────────────────────────────
  "notif.title": "通知",
  "notif.empty": "通知はまだありません",
  "notif.new": "{count} 件の新着",
  "notif.markAllRead": "すべて既読にする",
  "notif.reaction": "{actor} があなたの投稿にリアクションしました",
  "notif.comment": "{actor} があなたの投稿にコメントしました",
  "notif.reply": "{actor} があなたのコメントに返信しました",
  "notif.suggestion": "{actor} が翻訳を提案しました",
  "notif.suggestionApproved": "あなたの翻訳提案が承認されました",
  "notif.follow": "{actor} があなたをフォローしました",
  "notif.medal": "メダルを獲得しました！",
  "notif.seasonStart": "新しいシーズンが始まりました！",
  "notif.seasonEnd": "シーズンが終了しました",
  "notif.monthlyWinner": "{month}の月間ベストミームが選ばれました！",
  "notif.system": "システムからのお知らせ",
  "notif.rewardGranted": "新しい報酬を獲得しました！",
  "notif.directMessage": "{actor}からメッセージが届きました",
  "notif.forward": "{actor}がミームを共有しました",
  "notif.rankChange": "ランキングが変動しました！",
  "notif.translationRequest": "{actor}が翻訳をリクエストしました",
  "notif.new_notification": "新しい通知",

  // ── Sidebar ────────────────────────────────────────────────────────────────
  "sidebar.countryRankings": "国別ランキング",
  "sidebar.viewAll": "すべて見る →",
  "sidebar.hotMemes": "人気のミーム",
  "sidebar.topCreators": "トップクリエイター",
  "sidebar.trending": "トレンド",
  "sidebar.emptyCountry": "ミームを投稿して自分の国を代表しよう！",
  "sidebar.emptyMemes": "最初のミームを投稿しよう！",
  "sidebar.emptyCreators": "最初のクリエイターになろう！",
  "sidebar.climbRanks": "投稿＆翻訳でランクアップ！",

  // ── Footer ─────────────────────────────────────────────────────────────────
  "footer.terms": "利用規約",
  "footer.privacy": "プライバシーポリシー",
  "footer.cookies": "Cookieポリシー",
  "footer.rules": "コミュニティルール",
  "footer.copyright": "著作権 / DMCA",
  "footer.allRights": "© {year} mimzy. All rights reserved.",

  // ── Feed ───────────────────────────────────────────────────────────────────
  "feed.empty": "まだ投稿がありません。最初のミームをシェアしよう！",
  "feed.endOfFeed": "すべて表示しました",
  "feed.fetchError": "投稿の読み込みに失敗しました。",
  "feed.viewAllComments": "{count} 件のコメントをすべて見る",
  "feed.deletePost": "投稿を削除",
  "feed.deleteConfirm": "この投稿を削除しますか？この操作は取り消せません。",
  "feed.deleting": "削除中...",
  "feed.linkCopied": "リンクをコピーしました！",
  "feed.bookmarked": "ブックマークしました！",
  "feed.bookmarkRemoved": "ブックマークを解除しました",
  "feed.aiTranslation": "AI翻訳",
  "feed.requestTranslation": "翻訳をリクエスト",
  "feed.translationSent": "翻訳リクエストを送信しました！",
  "feed.translationFailed": "リクエストの送信に失敗しました",
  "feed.postDeleted": "投稿が削除されました",
  "feed.deleteFailed": "投稿の削除に失敗しました",

  // ── Feed Filters ───────────────────────────────────────────────────────────
  "filter.trending": "トレンド",
  "filter.recent": "新着",
  "filter.top": "人気",
  "filter.allCountries": "すべての国",
  "filter.allLanguages": "すべての言語",
  "filter.allCategories": "すべてのカテゴリ",
  "filter.usa": "アメリカ",
  "filter.korea": "韓国",
  "filter.japan": "日本",
  "filter.china": "中国",
  "filter.mexico": "メキシコ",
  "filter.uk": "イギリス",
  "filter.india": "インド",
  "filter.spain": "スペイン",
  "filter.english": "English",
  "filter.korean": "한국어",
  "filter.japanese": "日本語",
  "filter.chinese": "中文",
  "filter.spanish": "Español",
  "filter.reaction": "リアクション",
  "filter.comic": "コミック",
  "filter.screenshot": "スクリーンショット",
  "filter.classic": "クラシック",
  "filter.anime": "アニメ",

  // ── Comments ───────────────────────────────────────────────────────────────
  "comment.add": "コメントを入力... @でメンションできます",
  "comment.posting": "投稿中...",
  "comment.post": "コメント",
  "comment.reply": "返信",
  "comment.replyPlaceholder": "返信を入力... @でメンション",
  "comment.edit": "編集",
  "comment.delete": "削除",
  "comment.deleteConfirm": "このコメントを削除しますか？",
  "comment.save": "保存",
  "comment.cancel": "キャンセル",
  "comment.report": "報告",
  "comment.reportTitle": "コメントを報告",
  "comment.reportReason": "理由を選択...",
  "comment.reportDetails": "詳細（任意）...",
  "comment.reportSubmit": "送信",
  "comment.empty": "コメントはまだありません。最初にコメントしよう！",
  "comment.loginToComment": "コメントするにはログインしてください。",
  "comment.you": "あなた",
  "comment.translate": "翻訳",
  "comment.original": "原文",
  "comment.posted": "コメントを投稿しました",
  "comment.deleted": "コメントを削除しました",
  "comment.updated": "コメントを更新しました",
  "comment.reportSubmitted": "報告を送信しました。ありがとうございます。",
  "comment.failedPost": "コメントの投稿に失敗しました",
  "comment.failedDelete": "コメントの削除に失敗しました",
  "comment.failedUpdate": "コメントの更新に失敗しました",
  "comment.failedReport": "報告の送信に失敗しました",
  "comment.failedReply": "返信の投稿に失敗しました",
  "comment.spam": "スパム",
  "comment.harassment": "嫌がらせ",
  "comment.hateSpeech": "ヘイトスピーチ",
  "comment.violence": "暴力",
  "comment.sexualContent": "性的コンテンツ",
  "comment.misinformation": "誤情報",
  "comment.copyrightReport": "著作権",
  "comment.other": "その他",
  "comment.edited": "編集済み",
  "comment.sortTop": "人気",
  "comment.sortNewest": "最新",
  "comment.sortOldest": "古い順",
  "comment.showMoreReplies": "他 {count} 件の返信を表示",
  "comment.collapseReplies": "返信を折りたたむ",

  // ── Mention ────────────────────────────────────────────────────────────────
  "mention.searching": "検索中...",
  "mention.noUsers": "ユーザーが見つかりません",

  // ── Post Detail ────────────────────────────────────────────────────────────
  "post.comments": "コメント",
  "post.discussion": "翻訳を提案",
  "post.cultureNote": "カルチャーノート",
  "post.sharePost": "投稿をシェア",
  "post.reportPost": "投稿を報告",
  "post.deletePost": "投稿を削除",
  "post.translated": "翻訳済み",
  "post.original": "原文",
  "post.views": "閲覧",
  "post.shares": "共有",
  "post.edited": "（編集済み）",
  "post.editPost": "投稿を編集",
  "post.editSaved": "変更を保存しました",
  "post.saveChanges": "変更を保存",
  "post.title": "タイトル",
  "post.titleRequired": "タイトルは必須です",
  "post.titlePlaceholder": "投稿タイトル",
  "post.body": "本文",
  "post.bodyPlaceholder": "投稿本文（任意）",
  "post.category": "カテゴリ",
  "post.noCategory": "カテゴリなし",
  "post.tags": "タグ",
  "post.addTags": "タグを追加...",
  "post.forward": "転送",
  "post.forwardSent": "転送しました！",
  "post.searchUsers": "ユーザーを検索...",
  "post.noUsersFound": "ユーザーが見つかりません",
  "post.noRecentConversations": "最近の会話がありません",
  "post.addMessage": "メッセージを追加（任意）",
  "post.sendForward": "送信",
  "feed.save": "保存",

  // ── Discussion / Suggestion ────────────────────────────────────────────────
  "discussion.description": "このミームをもっと上手に翻訳できますか？あなたのバージョンを提案してください！",
  "discussion.placeholder": "翻訳の提案を入力...",
  "discussion.post": "送信",
  "discussion.posting": "送信中...",
  "discussion.empty": "翻訳の提案はまだありません。最初に提案してみましょう！",
  "discussion.translate": "翻訳",
  "discussion.original": "原文",
  "discussion.loginToPost": "翻訳を提案するにはログインしてください。",

  // ── Settings ───────────────────────────────────────────────────────────────
  "settings.title": "設定",
  "settings.profile": "プロフィール",
  "settings.account": "アカウント",
  "settings.notifications": "通知",
  "settings.language": "言語",
  "settings.about": "このアプリについて",
  "settings.profileSettings": "プロフィール設定",
  "settings.profilePhoto": "プロフィール写真",
  "settings.changePhoto": "変更",
  "settings.displayName": "表示名",
  "settings.bio": "自己紹介",
  "settings.country": "国 / チーム",
  "settings.selectCountry": "国を選択してください",
  "settings.saveChanges": "変更を保存",
  "settings.saving": "保存中...",
  "settings.saved": "変更を保存しました！",
  "settings.saveFailed": "変更の保存に失敗しました",
  "settings.appearance": "外観",
  "settings.darkMode": "ダークモード",
  "settings.accountInfo": "アカウント情報",
  "settings.email": "メールアドレス",
  "settings.username": "ユーザー名",
  "settings.role": "ロール",
  "settings.password": "パスワード",
  "settings.passwordPlaceholder": "••••••••",
  "settings.notifPrefs": "通知設定",
  "settings.notifReactions": "投稿へのリアクション",
  "settings.notifComments": "投稿へのコメント",
  "settings.notifFollowers": "新しいフォロワー",
  "settings.notifSuggestions": "翻訳提案",
  "settings.notifSeason": "シーズンの更新",
  "settings.notifEmail": "メールダイジェスト",
  "settings.langTranslation": "言語と翻訳",
  "settings.interfaceLang": "インターフェース言語",
  "settings.interfaceLangDesc": "ボタン、メニュー、UI要素の表示言語",
  "settings.preferredLang": "優先翻訳言語",
  "settings.preferredLangDesc": "ミーム閲覧時に最初に表示される翻訳言語",
  "settings.autoTranslation": "翻訳を自動表示",
  "settings.autoTranslationDesc": "ミーム画像に翻訳テキストを自動的に表示する",
  "settings.savePrefs": "設定を保存",
  "settings.aboutTitle": "mimzyについて",
  "settings.version": "バージョン",
  "settings.aboutDesc": "AIを活用したグローバルミーム翻訳＆オリンピックスタイルの国別対抗プラットフォーム。",

  // ── Auth ───────────────────────────────────────────────────────────────────
  "auth.signIn": "ログイン",
  "auth.signUp": "アカウント作成",
  "auth.email": "メールアドレス",
  "auth.password": "パスワード",
  "auth.confirmPassword": "パスワード確認",
  "auth.username": "ユーザー名",
  "auth.displayName": "表示名（任意）",
  "auth.country": "国",
  "auth.selectCountry": "国を選択してください",
  "auth.signingIn": "ログイン中...",
  "auth.creatingAccount": "アカウント作成中...",
  "auth.invalidCredentials": "メールアドレスまたはパスワードが正しくありません",
  "auth.passwordMismatch": "パスワードが一致しません",
  "auth.passwordMinLength": "パスワードは6文字以上で入力してください",
  "auth.selectCountryError": "国を選択してください",
  "auth.agreeTerms": "利用規約とプライバシーポリシーに同意してください",
  "auth.agreeTermsLabel": "以下に同意します：",
  "auth.termsAndPrivacy": "利用規約とプライバシーポリシー",
  "auth.noAccount": "アカウントをお持ちでない方",
  "auth.haveAccount": "すでにアカウントをお持ちの方",
  "auth.orContinueWith": "または以下で続ける",
  "auth.continueWithGoogle": "Googleで続ける",
  "auth.signUpFailed": "アカウント作成に失敗しました",
  "auth.passwordPlaceholder6": "6文字以上",
  "auth.reenterPassword": "パスワード再入力",
  "auth.signUpSuccessLoginFailed": "アカウントは作成されましたが、自動ログインに失敗しました。ログインしてください。",
  "auth.forgotPassword": "パスワードをお忘れですか？",
  "auth.forgotPasswordDescription": "メールアドレスを入力してください。リセットリンクを生成します。",
  "auth.resetLinkSent": "該当するメールアドレスのアカウントがあれば、リセットリンクが生成されました。サーバーログをご確認ください。",
  "auth.backToSignIn": "ログインに戻る",
  "auth.sending": "送信中...",
  "auth.sendResetLink": "リセットリンクを送信",
  "auth.rememberPassword": "パスワードを覚えていますか？",
  "auth.resetPassword": "パスワードリセット",
  "auth.enterNewPassword": "新しいパスワードを入力してください。",
  "auth.passwordResetSuccess": "パスワードがリセットされました！ログインページに移動します...",
  "auth.newPassword": "新しいパスワード",
  "auth.resetting": "リセット中...",

  // ── Upload ─────────────────────────────────────────────────────────────────
  "upload.title": "ミームをシェア",
  "upload.subtitle": "ミームをアップロードすると、7言語に自動翻訳されます",
  "upload.dragDrop": "ここにミームをドラッグ＆ドロップ",
  "upload.orBrowse": "またはクリックして選択",
  "upload.maxSize": "最大10MB · JPG, PNG, GIF, WebP",
  "upload.memeTitle": "タイトル",
  "upload.memeTitlePlaceholder": "キャッチーなタイトルをつけよう...",
  "upload.sourceLanguage": "元の言語",
  "upload.category": "カテゴリ",
  "upload.tags": "タグ",
  "upload.tagsPlaceholder": "タグを追加（Enterで確定）",
  "upload.submit": "アップロード＆翻訳",
  "upload.uploading": "アップロード中...",
  "upload.success": "ミームをアップロードしました！",
  "upload.failed": "ミームのアップロードに失敗しました",
  "upload.changeImage": "画像を変更",

  // ── Leaderboard ────────────────────────────────────────────────────────────
  "leaderboard.title": "ランキング",
  "leaderboard.countries": "国別",
  "leaderboard.creators": "クリエイター",
  "leaderboard.memes": "ミーム",
  "leaderboard.rank": "順位",
  "leaderboard.score": "スコア",
  "leaderboard.posts": "投稿数",

  // ── Season ─────────────────────────────────────────────────────────────────
  "season.title": "シーズン",
  "season.openSeason": "オープンシーズン",
  "season.allTimeRankings": "通算ランキング",
  "season.left": "残り {time}",
  "season.ended": "終了",
  "season.leading": "首位: {flag} {name}",
  "season.voteOpen": "最終投票受付中 — 投票しよう！",
  "season.champion": "{year} チャンピオン — {country}",
  "season.dashboard": "シーズンダッシュボード",
  "season.judgingDescription": "最終投票受付中！今年のベストミームに投票しよう。",
  "season.activeDescription": "毎月最も「いいね」されたミームが月間優勝者に選ばれます。",
  "season.pastDescription": "過去の月間優勝者とシーズン結果を表示します。",
  "season.memeOfTheMonth": "{month} 月間ベストミーム",
  "season.noWinners": "まだ月間優勝者がいません。",
  "season.winnersDescription": "優勝者は毎月初めに選出されます。",
  "season.viewAll": "すべて表示 →",
  "season.voteNow": "投票する",
  "season.howItWorks": "仕組み",
  "season.step1": "毎月最高のミームを投稿して「いいね」を競いましょう",
  "season.step2": "毎月最も「いいね」を集めたミームが月間チャンピオンに選ばれます",
  "season.step3": "年末に12人の月間チャンピオンがグランドトーナメントに出場します",
  "season.step4": "最終チャンピオンはチャンピオンの称号と永遠の栄光を手にします！",
  "season.seasonPeriod": "シーズン期間",
  "season.monthlyWinners": "月間チャンピオン",
  "season.currentProgress": "現在の進捗",
  "season.monthsCompleted": "12ヶ月中 {count}ヶ月完了",
  "season.monthsRemaining": "残り {count}ヶ月",

  // ── Common ─────────────────────────────────────────────────────────────────
  "common.loading": "読み込み中...",
  "common.error": "エラーが発生しました",
  "common.save": "保存",
  "common.cancel": "キャンセル",
  "common.delete": "削除",
  "common.confirm": "確認",
  "common.close": "閉じる",
  "common.retry": "もう一度試す",
  "common.back": "戻る",
  "common.next": "次へ",
  "common.submit": "送信",
  "common.search": "検索",
  "common.noResults": "結果が見つかりません",
  "common.viewAll": "すべて見る",
  "common.seeMore": "もっと見る",
  "common.justNow": "たった今",

  // ── Search ─────────────────────────────────────────────────────────────────
  "search.title": "検索",
  "search.placeholder": "ミーム、ユーザー、タグを検索...",
  "search.noResults": "「{query}」の検索結果はありません",
  "search.results": "「{query}」の検索結果",

  // ── Messages ───────────────────────────────────────────────────────────────
  "messages.title": "メッセージ",
  "messages.empty": "まだメッセージはありません",
  "messages.newMessage": "新しいメッセージ",
  "messages.typePlaceholder": "メッセージを入力...",
  "messages.send": "送信",
  "messages.startHint": "ユーザーのプロフィールで「メッセージ」をクリックして会話を始めましょう",
  "messages.searchPlaceholder": "会話を検索...",
  "messages.filterAll": "すべて",
  "messages.filterUnread": "未読",
  "messages.noResults": "該当する会話はありません",

  // ── Bookmarks ──────────────────────────────────────────────────────────────
  "bookmarks.title": "ブックマーク",
  "bookmarks.empty": "ブックマークはまだありません。ミームを保存してここで見よう！",

  // ── Admin ──────────────────────────────────────────────────────────────────
  "admin.title": "mimzy 管理画面",
  "admin.dashboard": "ダッシュボード",
  "admin.users": "ユーザー",
  "admin.posts": "投稿",
  "admin.translations": "翻訳",
  "admin.monthlyWinners": "月間優勝者",
  "admin.seasonManagement": "シーズン管理",
  "admin.createSeason": "シーズン {year} を作成",
  "admin.createBetaSeason": "ベータシーズンを作成",
  "admin.activeSeason": "アクティブなシーズン",
  "admin.noActiveSeason": "アクティブなシーズンはありません",
  "admin.selectMonthlyWinner": "月間優勝者を選ぶ",

  // ── Cookie Consent ─────────────────────────────────────────────────────────
  "cookie.message": "より良い体験のためにCookieを使用しています。",
  "cookie.accept": "同意する",
  "cookie.decline": "拒否する",

  // ── Culture Note ───────────────────────────────────────────────────────────
  "cultureNote.title": "カルチャーノート",
  "cultureNote.aiGenerated": "AIが生成した文化的背景情報",

  // ── Welcome ────────────────────────────────────────────────────────────────
  "welcome.title": "mimzyへようこそ！",
  "welcome.subtitle": "プロフィールを設定して始めよう",

  // ── User Profile ───────────────────────────────────────────────────────────
  "profile.posts": "投稿",
  "profile.followers": "フォロワー",
  "profile.following": "フォロー中",
  "profile.follow": "フォロー",
  "profile.unfollow": "フォロー解除",
  "profile.editProfile": "プロフィールを編集",
  "profile.message": "メッセージ",
  "profile.noPosts": "まだ投稿がありません",
  "profile.joined": "{date} に登録",
  "hallOfFame.title": "殿堂",
  "hallOfFame.subtitle": "最高のミームとクリエイターたちが永遠に記録されます。",
  "hallOfFame.noChampions": "まだチャンピオンがいません。",
  "hallOfFame.firstChampion": "最初のチャンピオンは現在のシーズン終了時に選出されます。",
  "postDetail.notFound": "投稿が見つかりません",
  "postDetail.loadError": "投稿の読み込みに失敗しました。",
  "profile.notFound": "ユーザーが見つかりません",

  // ── Battle ──────────────────────────────────────────────────────────────────
  "battle.title": "ミームバトル",
  "battle.tapToVote": "面白い方をタップ！",
  "battle.winner": "勝利！",
  "battle.bonusPoints": "+5 ボーナス",
  "battle.stop": "やめる",
  "battle.noChallengers": "挑戦者がいません！",
  "battle.hotBattle": "ホットバトルミーム",

  // ── Winner Popup ──────────────────────────────────────────────────────────
  "winner.champion": "チャンピオン",
  "winner.monthlyWinner": "月間優勝者",
  "winner.memeOfTheYear": "年間ベストミーム",
  "winner.countryOfTheYear": "年間ベスト国",
  "winner.viewResults": "結果を見る",
  "winner.viewPost": "見る",
  "winner.dismiss": "閉じる",
  "vote.upvote": "賛成",
  "vote.downvote": "反対",
  "vote.score": "スコア",

  // ── Tournament ─────────────────────────────────────────────────────────────
  "tournament.title": "年末トーナメント",
  "tournament.subtitle": "12人の月間優勝者が対決。今年のミームに投票しよう！",
  "tournament.memeOfTheYear": "今年のミーム",
  "tournament.quarterfinals": "準々決勝",
  "tournament.semifinals": "準決勝",
  "tournament.final": "決勝",
  "tournament.live": "ライブ",
  "tournament.waitingFor": "{round}の結果待ち...",
  "tournament.voteToSee": "投票すると結果が見れます",
  "tournament.totalVotes": "{count}票",

  // ── Leaderboard Dashboard ─────────────────────────────────────────────────
  "leaderboard.countryCompetition": "国別対抗戦",
  "leaderboard.totalReactions": "総リアクション",
  "leaderboard.countriesActive": "参加国数",
  "leaderboard.scoreDistribution": "スコア分布",
  "leaderboard.allTimeRankings": "全期間のコミュニティ活動に基づくランキング",
  "leaderboard.seasonRankings": "今シーズンのグローバルランキング",
  "leaderboard.realtimeNotice": "全期間リアルタイムランキング — シーズンランキングは近日公開！",
  "leaderboard.noActivity": "まだ活動がありません — ミームを投稿してランクインしよう！",

  // ── Dashboard ───────────────────────────────────────────────────────────────
  "dashboard.title": "国別コンペティション",
  "dashboard.subtitle": "リアルタイムグローバルランキング",
  "dashboard.totalPosts": "総ミーム数",
  "dashboard.totalReactions": "総リアクション数",
  "dashboard.activeCountries": "参加国",
  "dashboard.topMeme": "今月のトップミーム",
  "dashboard.countryRankings": "国別ランキング",
  "dashboard.monthlyTrend": "月間優勝者",
  "dashboard.noData": "データがまだありません",
  "dashboard.viewLeaderboard": "ランキング全体を見る",
  "dashboard.viewTournament": "トーナメントを見る",

  // ── Monthly Voting ─────────────────────────────────────────────────────
  "monthly.votingStatus": "月間投票",
  "monthly.daysLeft": "残り{days}日",
  "monthly.totalVotesThisMonth": "今月の総リアクション {count}件",
  "monthly.viewFullRankings": "全ランキング",
  "monthly.winnersTitle": "月間チャンピオン",
  "monthly.winnersSubtitle": "毎月選ばれたベストミーム",
  "monthly.noWinnersYet": "まだ月間優勝者はいません",
  "monthly.reactions": "リアクション",

  "bookmarks.subtitle": "保存したミーム",
  "bookmarks.explore": "ミームを探す",

  "search.recentSearches": "最近の検索",
  "search.memes": "ミーム",
  "search.users": "ユーザー",
  "search.reactions": "リアクション",
  "search.comments": "コメント",
  "search.posts": "投稿",
  "search.followers": "フォロワー",
  "search.noResultsTitle": "「{query}」の検索結果はありません",
  "search.noResultsHint": "別のキーワードを試すか、スペルを確認してください",
  "search.suggestions": "提案",
  "search.suggestionBroader": "より広いまたは短い検索語を使用してください",
  "search.suggestionTags": "タグで検索（例：funny、anime）",
  "search.suggestionUsername": "ユーザー名または国で検索",

    "tournament.roundOf16": "16強",
  "tournament.roundOf8": "準々決勝",
  "tournament.semiFinals": "準決勝",
  "tournament.grandFinal": "決勝",
  "tournament.started": "開始済み",
  "tournament.liveNow": "ライブ",
  "tournament.startsIn": "{time}後に開始",
  "tournament.bracketNotCreated": "Tournament bracket will be created when the season starts",

  "battle.tapToContinue": "タップして続ける",
  "battle.nextChallenger": "次の挑戦者が登場...",
  "battle.streak.nicePick": "ナイスピック！",
  "battle.streak.onARoll": "連勝中！",
  "battle.streak.hatTrick": "ハットトリック！",
  "battle.streak.unstoppable": "止められない！",
  "battle.streak.domination": "圧倒的！",
  "battle.streak.legendary": "レジェンド！",
  "battle.streak.godMode": "ゴッドモード！",

  "feed.translated": "翻訳済み",
  "feed.original": "オリジナル",

  "auth.joinPlatform": "グローバルミーム翻訳プラットフォームに参加",
  "auth.platformTagline": "グローバルミーム翻訳プラットフォーム",
  "auth.passwordMinLength8": "パスワードは8文字以上必要です",
  "auth.passwordPlaceholder8": "8文字以上",

  // ── Scoring Explanation ───────────────────────────────────────────────
  "scoring.howItWorks": "スコアの仕組み",
  "scoring.countryTitle": "国スコア",
  "scoring.countryDesc": "その国の全投稿が受けたエンゲージメントの合計",
  "scoring.creatorTitle": "クリエイタースコア",
  "scoring.creatorDesc": "投稿が受けたエンゲージメントに基づく個人スコア",
  "scoring.creatorFormula": "リアクション + コメント \u00D7 2 + シェア \u00D7 3",
  "scoring.memeTitle": "ミームスコア",
  "scoring.memeDesc": "時間減衰付きの個別ミームランキング",
  "scoring.memeFormula": "リアクション \u00D7 2 + コメント \u00D7 3 + 翻訳 \u00D7 5 + 閲覧 \u00D7 0.01",
  "scoring.memeDecay": "新しいコンテンツが競争力を保つよう、時間とともにスコアが減少します",
  "scoring.medalsTitle": "メダル",
  "scoring.medalsDesc": "各カテゴリの上位3名が金、銀、銅メダルを獲得",
  "scoring.gotIt": "了解！",

  // ── Hero Banner ──────────────────────────────────────────────────────
  "hero.tagline": "あなたのミーム。7言語。一つのグローバルステージ。",
  "hero.subtitle": "\u30DF\u30FC\u30E0\u3092\u30A2\u30C3\u30D7\u3059\u308B\u3068AI\u304C\u77AC\u6642\u306B7\u8A00\u8A9E\u306B\u7FFB\u8A33",
  "hero.cta": "\u30B0\u30ED\u30FC\u30D0\u30EB\u30DF\u30FC\u30E0\u30AA\u30EA\u30F3\u30D4\u30C3\u30AF\u306B\u53C2\u52A0",
  "hero.stat.languages": "7\u8A00\u8A9E",
  "hero.stat.champions": "\u6708\u9593\u30C1\u30E3\u30F3\u30D4\u30AA\u30F3",
  "hero.stat.battle": "\u56FD vs \u56FD",

  // ── Onboarding ───────────────────────────────────────────────────────
  "onboarding.skip": "スキップ",
  "onboarding.next": "次へ",
  "onboarding.back": "戻る",
  "onboarding.step1Title": "mimzyへようこそ！",
  "onboarding.step1Desc": "世界初のグローバルミームコンペティション",
  "onboarding.step1Detail": "ミームは言語を超える。ここでは国をつなぐ。",
  "onboarding.step2Title": "使い方",
  "onboarding.step2Upload": "ミームをアップロード",
  "onboarding.step2Translate": "AIが7言語に翻訳",
  "onboarding.step2Compete": "世界と競争",

  // ── Rank / Level System ─────────────────────────────────────────────────────
  "rank.level": "レベル",
  "rank.tier": "ティア",
  "rank.xpToNext": "次のレベルまで {xp} XP",
  "rank.totalXp": "合計XP",
  "rank.iron": "アイアン",
  "rank.bronze": "ブロンズ",
  "rank.silver": "シルバー",
  "rank.gold": "ゴールド",
  "rank.platinum": "プラチナ",
  "rank.diamond": "ダイヤモンド",
  "rank.master": "マスター",
  "rank.challenger": "チャレンジャー",

  // ── Country Race Widget ────────────────────────────────────────────────────
  "countryRace.leading": "あなたの国がリードしています！",
  "countryRace.pointsAhead": "{country}より{points}ポイント先行中",
  "countryRace.pointsBehind": "{country}より{points}ポイント遅れています",
  "countryRace.closeGap": "ミームを投稿して差を縮めよう！",
  "countryRace.keepLead": "リードを維持しよう！",
  "countryRace.uploadHelp": "国のためにミームを投稿",
  "countryRace.vs": "vs",

  // ── Rank Change Notifications ─────────────────────────────────────────────
  "rankChange.movedUp": "あなたの国が{rank}位に上昇！🎉",
  "rankChange.movedDown": "あなたの国が{rank}位に下落 — 投稿の時間！",
  "rankChange.overtook": "{country1}が{country2}を追い抜きました！",

  // ── Country Champion Badge ────────────────────────────────────────────────
  "countryChampion.badge": "1位の国のメンバー",

  "onboarding.step3BattleTitle": "ミームバトル",
  "onboarding.step3BattleDesc": "どっちのミームが面白いか投票しよう！投票でベストミームをランク付けし、国のポイントを稼ごう。",
  "onboarding.step4TranslationTitle": "翻訳トグル",
  "onboarding.step4TranslationDesc": "オリジナルと翻訳の表示を切り替え。元のユーモアを保ちながら自分の言語でミームを楽しもう。",
  "onboarding.step5RankTitle": "ランクアップシステム",
  "onboarding.step5RankDesc": "アップロード、リアクション、バトルでXPを獲得。アイアンからチャレンジャーまで — ゲームのように！",
  "onboarding.step6Title": "国のためにポイントを稼ごう",
  "onboarding.step6Desc": "すべてのリアクション、コメント、シェアがポイントに",
  "onboarding.step6Detail": "ミームへのエンゲージメントが多いほど国のランキングが上がる！",
  "onboarding.step7Title": "代表する準備はできた？",
  "onboarding.step7Cta": "最初のミームを投稿",
  "onboarding.step7Browse": "まず見てみる",
  "nav.battle": "バトル",
  "sidebar.yourCountry": "あなたの国: {flag} {name} — ランク #{rank} — スコア: {score}",
  "sidebar.uploadToClimb": "ミームを投稿してランクを上げよう！",
  "filter.myCountry": "自分の国",
  "feed.report": "報告",
  "feed.reportTitle": "投稿を報告",
  "feed.reportSubmitted": "報告が送信されました",
  "feed.reportFailed": "報告の送信に失敗しました",
  "feed.reportSpam": "スパム",
  "feed.reportInappropriate": "不適切",
  "feed.reportCopyright": "著作権",
  "feed.reportOther": "その他",

  // ── Block ──────────────────────────────────────────────────────────────────
  "block.block": "ブロック",
  "block.unblock": "ブロック解除",
  "block.confirmTitle": "このユーザーをブロックしますか？",
  "block.confirmMessage": "ブロックされたユーザーはあなたの投稿を見たり、メッセージを送ったりできなくなります。",
  "block.blockedUsers": "ブロックリスト",
  "block.noBlockedUsers": "ブロック中のユーザーはいません",
  "block.userBlocked": "ユーザーをブロックしました",
  "block.userUnblocked": "ブロックを解除しました",

  // ── Ban ────────────────────────────────────────────────────────────────────
  "ban.banned": "アカウント停止",
  "ban.reason": "理由",
  "ban.duration": "期間",
  "ban.permanent": "永久",
  "ban.days": "{count}日間",
  "ban.bannedMessage": "アカウントが停止されました。",
  "ban.contactSupport": "誤りだと思われる場合はサポートにお問い合わせください。",

  // ── Karma ──────────────────────────────────────────────────────────────────
  "karma.postKarma": "投稿カルマ",
  "karma.commentKarma": "コメントカルマ",

  // ── Search (filters) ──────────────────────────────────────────────────────
  "search.timeRange": "期間",
  "search.24h": "24時間",
  "search.week": "今週",
  "search.month": "今月",
  "search.year": "今年",
  "search.allTime": "全期間",
  "search.sortBy": "並び替え",
  "search.relevance": "関連度",
  "search.top": "人気",
  "search.newest": "最新",

  // ── Forward ────────────────────────────────────────────────────────────────
  "forward.title": "送信先",
  "forward.searchUsers": "ユーザーを検索...",
  "forward.recentConversations": "最近の会話",
  "forward.addMessage": "メッセージを追加...",
  "forward.sent": "送信完了！",
  "forward.send": "送信",

  // ── Badges ────────────────────────────────────────────────────────────────
  "badge.FIRST_TRANSLATION": "初翻訳者",
  "badge.UPLOAD_10": "積極的な投稿者",
  "badge.UPLOAD_50": "ミームの伝説",
  "badge.REACTION_100": "人気クリエイター",
  "badge.BATTLE_10W": "バトルベテラン",
  "badge.STREAK_7": "連続投稿中",
  "badge.STREAK_30": "無敵のクリエイター",
  "badge.earned": "バッジ獲得: {name}!",
  "sidebar.nudge": "{country}まであと{diff}点!",
  "sidebar.nudgePost": "投稿して#{rank}位を目指そう",
  "sidebar.streak": "🔥 {n}日連続投稿中!",
  "sidebar.streakStart": "今日投稿してストリークを始めよう!",
  "notif.RANK_CHANGE": "{country}に追い抜かれました！",
  "notif.BADGE_EARNED": "バッジを獲得しました！",
  "notif.STREAK_BONUS": "🔥 {n}日ストリークボーナス！",
  "notif.TRANSLATION_REQUEST": "{actor}が翻訳をリクエストしました！",
  "notif.BATTLE_EVENT": "新しいバトルイベント: {title}",
  "feed.following": "フォロー中",
  "feed.followingEmpty": "フォローすると投稿が表示されます！",
  "countryBoard.title": "{country}コミュニティボード",
  "countryBoard.placeholder": "あなたの国のために何か書いてください...",
  "countryBoard.empty": "まだ投稿がありません。",

  // ── Championship ──────────────────────────────────────────────────────────
  "nav.championship": "チャンピオンシップ",
  "championship.title": "年末チャンピオンシップ",
  "championship.subtitle": "各国の代表が究極のタイトルを争います",
  "championship.notActive": "現在チャンピオンシップは開催されていません。",
  "championship.comingDecember": "チャンピオンシップは毎年12月に開催されます。",
  "championship.phase.nomination": "ノミネート",
  "championship.phase.vote": "投票",
  "championship.phase.upload": "アップロード",
  "championship.phase.battle": "バトル",
  "championship.phase.results": "結果",
  "championship.nominations": "候補者一覧",
  "championship.voteForRep": "代表に投票しよう",
  "championship.candidatesCount": "名の候補者",
  "championship.seasonScore": "シーズンスコア",
  "championship.votes": "票",
  "championship.voteFor": "投票",
  "championship.voting": "投票中...",
  "championship.voted": "投票済み",
  "championship.alreadyVoted": "この国ではすでに投票済みです",
  "championship.elected": "当選",
  "championship.runnerUp": "次点",
  "championship.eliminated": "敗退",
  "championship.substitute": "代理出場",
  "championship.uploadPhase": "チャンピオンシップ投稿期間",
  "championship.uploadDesc": "選出された代表がチャンピオンシップ投稿をアップロード中です。",
  "championship.submittedPosts": "提出済み投稿",
  "championship.battlePhase": "チャンピオンシップバトル",
  "championship.battleDesc": "最高のチャンピオンシップ投稿に投票しよう！複数の投稿に投票できます。",
  "championship.noPosts": "チャンピオンシップ投稿はまだありません。",
  "championship.noCandidates": "まだ候補者がいません。",
  "championship.results": "チャンピオンシップ結果",
  "championship.noResults": "まだ結果がありません。",
  "championship.otherRankings": "その他のランキング",
  "championship.banner.nomination": "候補者がノミネートされています！あなたの国の代表を確認しよう。",
  "championship.banner.vote": "今すぐ国の代表に投票しよう！",
  "championship.banner.upload": "代表がチャンピオンシップ投稿をアップロード中です。",
  "championship.banner.battle": "チャンピオンシップバトル開催中！最高の投稿に投票しよう！",
  "championship.banner.active": "チャンピオンシップ開催中！",
  "championship.banner.ctaVote": "投票する",
  "championship.banner.ctaBattle": "投票する",
  "championship.banner.ctaView": "見る",
  "championship.projectedTop8": "現在の進出予想Top 8カ国",
  "championship.top8Explanation": "ユーザー平均スコア上位8カ国が12月チャンピオンシップに進出します。",
  "championship.lastYearResults": "昨年のチャンピオンシップ結果",
  "championship.uploadProgress": "アップロード進捗",
  "championship.uploadCta": "投稿をアップロード",
  "championship.totalVoteCast": "総投票数",
  "championship.champion": "チャンピオン",
  "championship.secondPlace": "準優勝",
  "championship.thirdPlace": "3位",
  "championship.howItWorks": "シーズンの仕組み",
  "championship.howItWorksStep1Title": "月間コンペティション",
  "championship.howItWorksStep1Desc": "毎月クリエイターがミームを投稿します。最も人気のあるミームが「今月のミーム」に選ばれます。",
  "championship.howItWorksStep2Title": "国別スコア",
  "championship.howItWorksStep2Desc": "すべての投稿が国にポイントを貢献します。公平性のためユーザーあたりの平均で算出されます。",
  "championship.howItWorksStep3Title": "トップ8予選",
  "championship.howItWorksStep3Desc": "ユーザーあたりの平均スコアが最も高い8カ国がチャンピオンシップに進出します。",
  "championship.howItWorksStep4Title": "チャンピオンシップ（12月）",
  "championship.howItWorksStep4Desc": "各予選国は投票で代表を選出し、代表がチャンピオンシップ投稿をアップロードします。",
  "championship.howItWorksStep5Title": "最終バトル",
  "championship.howItWorksStep5Desc": "全員がチャンピオンシップ投稿に投票します。勝者が今年のチャンピオンになります！",
  "tournament.movedToChampionship": "トーナメントが移動しました！",
  "tournament.movedDesc": "トーナメントは新しいチャンピオンシップシステムに置き換えられました。自動的に移動します。",
  "tournament.goToChampionship": "チャンピオンシップへ",
  "tournament.autoRedirect": "3秒後に自動移動...",
  "sidebar.top8Line": "チャンピオンシップ進出ライン",
  "sidebar.qualified": "あなたの国はチャンピオンシップに進出します！",
  "sidebar.championshipWidget": "チャンピオンシップ",
  "sidebar.viewChampionship": "チャンピオンシップを見る",
  "leaderboard.users": "ユーザー",
  "leaderboard.total": "合計",
  "leaderboard.perUser": "1人あたり",
  "leaderboard.qualificationLine": "チャンピオンシップ進出ライン",
  "leaderboard.nextRank": "次の順位",
  "leaderboard.myScore": "マイスコア",
  "leaderboard.weeklyMvp": "今週のMVP",
  "leaderboard.monthlyMvp": "今月のMVP",
  "leaderboard.countryMatchup": "国対国",
  "leaderboard.thisWeek": "今週",
  "leaderboard.liveActivity": "ライブ",
  "championship.yourCountryStatus": "あなたの国のステータス",
  "championship.eligible": "出場可能",
  "championship.needTop8": "Top 8必要",
  "onboarding.step7ChampionshipTitle": "年末チャンピオンシップ",
  "onboarding.step7ChampionshipDesc": "毎年12月、上位8カ国がチャンピオンシップで競います。あなたが代表になれるかも！",
  "onboarding.step7ChampCountry": "国を慎重に選んで — あなたが代表です！",
  "onboarding.step7ChampScore": "あなたの投稿が国のランキングにスコアを加えます",
  "onboarding.step7ChampBattle": "上位8カ国が代表を送りチャンピオンシップで対決",
  "onboarding.step7ChampTip": "ヒント: 国のスコアはユーザー平均で計算されるので、全てのアクティブメンバーが重要です！",
  "badge.CHAMPIONSHIP_CHAMPION": "チャンピオン",
  "badge.CHAMPIONSHIP_RUNNER_UP": "準優勝",
  "badge.CHAMPIONSHIP_REPRESENTATIVE": "チャンピオンシップ代表",
  "nav.community": "コミュニティ",
  "community.title": "コミュニティ",
  "community.subtitle": "世界中のミームクリエイターと交流・議論しよう",
  "community.discussion": "ディスカッション",
  "community.countryBoards": "国別掲示板",
  "community.emptyDiscussion": "まだディスカッションがありません。会話を始めましょう！",
  "community.emptyDiscussionSubtext": "世界中のクリエイターと考えを共有し、質問したり、ミームについて議論しましょう。",
  "community.members": "クリエイター",
  "community.noCountries": "まだアクティブな国がありません",
  "community.titlePlaceholder": "ディスカッションタイトル",
  "community.bodyPlaceholder": "何を考えていますか？（任意）",
  "community.postingAs": "あなたの国",
  "community.post": "投稿",
  "community.loginToPost": "ディスカッションに参加するにはログインしてください。",
  "community.attachImage": "画像",
  "community.uploading": "アップロード中...",
  "community.hotPosts": "人気のディスカッション",
  "community.startDiscussion": "上で最初の投稿を書きましょう！",
  "community.writePost": "投稿する",
  "community.newPost": "新規投稿",
  "community.noHotPosts": "まだ人気の投稿はありません",
  "community.guidelines": "コミュニティガイドライン",
  "community.rule1": "すべての文化を尊重しましょう",
  "community.rule2": "投稿は7言語に自動翻訳されます",
  "community.rule3": "ミームを共有して楽しみましょう",
  "community.rule4": "ヘイトスピーチやハラスメント禁止",
  "community.searchPlaceholder": "ディスカッションを検索...",
  "community.categoryAll": "すべて",
  "community.categoryFree": "自由",
  "community.categoryMeme": "ミーム",
  "community.categoryStocks": "株式",
  "community.categorySports": "スポーツ",
  "community.categoryPolitics": "政治",
  "community.selectCategory": "カテゴリ",
  "community.clearFilters": "フィルター解除",

  // ── Global translation indicator ───────────────────────────────────────────
  "translation.inProgress": "翻訳中…",
  "translation.starting": "翻訳を準備中…",
  "translation.postsCountSingular": "件の投稿",
  "translation.postsCountPlural": "件の投稿",
  "translation.failedShort": "失敗",
  "translation.defaultTitle": "投稿",

  // ── Landing Page ────────────────────────────────────────────────────────────
  "landing.howItWorks": "使い方",
  "landing.step1.title": "ミームをアップロード",
  "landing.step1.desc": "どの言語でも面白いミームを共有しよう",
  "landing.step2.title": "AIが翻訳",
  "landing.step2.desc": "7言語に即座に翻訳されます",
  "landing.step3.title": "グローバルバトル",
  "landing.step3.desc": "投票して競い合い、ランキングを上げよう",
  "landing.showcase.title": "今バズってるミーム",
  "landing.leaderboard.title": "国別ランキング",
  "landing.stats.posts": "ミーム",
  "landing.stats.translations": "翻訳",
  "landing.stats.countries": "国",
  "landing.stats.users": "クリエイター",
  "landing.cta.title": "世界デビューする？",
  "landing.cta.subtitle": "世界初のミーム翻訳プラットフォームに参加しよう",
  "landing.cta.button": "ミームの旅を始める — 無料",
  "landing.login": "ログイン",
  "landing.signup": "新規登録",
};

export default ja;
